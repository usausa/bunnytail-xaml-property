...
{
    private const string AttributeName = "BunnyTail.XamlProperty.DependencyPropertyAttribute";

    private const string DependencyObjectTypeName = "System.Windows.DependencyObject";
    private const string PropertyChangedEventArgsTypeName = "System.Windows.DependencyPropertyChangedEventArgs";
    ...
    private static Result<PropertyModel> GetPropertyModel(GeneratorAttributeSyntaxContext context)
    {
        ...
        var metadataOptions = default(string);
        ...
        foreach (var argument in context.Attributes[0].NamedArguments)
        {
            switch (argument.Key)
            {
                ...
                case "DefaultValueMember":
                    ...
                    break;
                case "Options":
                    metadataOptions = argument.Value.ToCSharpExpression();
                    break;
    ...
    private static (PropertyChangedModel? Model, DiagnosticInfo? Error) ResolvePropertyChanged(Compilation compilation, INamedTypeSymbol containingType, string methodName, ITypeSymbol propertyType, Location location)
    {
        ...
        foreach (var method in EnumerateCallbackMethods(compilation, containingType, methodName))
        {
            ...
            if (method.IsGenericMethod || !method.ReturnsVoid)
            {
                continue;
            }
            ...
            if (method.IsStatic)
            {
                if ((method.Parameters.Length == 2) &&
                    (method.Parameters[0].Type.ToDisplayString() == DependencyObjectTypeName) &&
                    (method.Parameters[1].Type.ToDisplayString() == PropertyChangedEventArgsTypeName))
                {
                    candidates.Add(new PropertyChangedModel(methodName, true, false, string.Empty, string.Empty));
                }
    ...
    private static (CoerceModel? Model, DiagnosticInfo? Error) ResolveCoerce(Compilation compilation, INamedTypeSymbol containingType, string methodName, ITypeSymbol propertyType, Location location)
    {
        ...
        foreach (var method in EnumerateCallbackMethods(compilation, containingType, methodName))
        {
            ...
            candidates.Add(new CoerceModel(methodName, false, method.IsStatic, method.Parameters[0].Type.ToDisplayString(TypeDisplayFormat)));
        }
    ...
    private static (ValidateModel? Model, DiagnosticInfo? Error) ResolveValidate(Compilation compilation, INamedTypeSymbol containingType, string methodName, ITypeSymbol propertyType, Location location)
    {
        ...
        foreach (var method in EnumerateCallbackMethods(compilation, containingType, methodName))
        {
            ...
            if (!SymbolEqualityComparer.Default.Equals(method.Parameters[0].Type, propertyType))
            {
                continue;
            }

            candidates.Add(new ValidateModel(methodName, false, method.Parameters[0].Type.ToDisplayString(TypeDisplayFormat)));
        }
    ...
    private static void BuildProperty(SourceBuilder builder, string className, PropertyModel property)
    {
        ...
        if (metadataArguments.Count > 0)
        {
            ...
            var metadataType = property.MetadataOptions is not null ? FrameworkPropertyMetadataTypeName : PropertyMetadataTypeName;
        ...
        if (property.Validate is not null)
        {
            ...
            if (property.Validate.IsMethodGroup)
            {
                builder.Append(property.Validate.MethodName);
            }
            else
            {
                builder
    ...
    private static List<string> MakeMetadataArguments(string className, PropertyModel property)
    {
        ...
        var useFramework = property.MetadataOptions is not null;
        ...
        if (useFramework)
        {
            arguments.Add(property.MetadataOptions!);
        }
    ...
    {
        if (changed.IsMethodGroup)
        {
            return changed.MethodName;
        }
